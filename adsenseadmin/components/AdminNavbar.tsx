...
export default function AdminNavbar({ children }: AdminNavbarProps) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('adsense-dark-mode');
    if (saved === 'true') {
      setDarkMode(true);
  ...
  const toggleDarkMode = () => {
    ...
    localStorage.setItem('adsense-dark-mode', String(next));
  };
  ...
  const getPageTitle = () => {
    if (pathname === '/dashboard/admin-users') return 'Admin Users';
    if (pathname === '/dashboard/adsense') return 'Adsense Dashboard';
    if (pathname === '/dashboard/invoice-requests') return 'Invoice Requests';
    if (pathname === '/reports/overview') return 'Overview';
    if (pathname === '/reports') return 'Reports';
    if (pathname === '/reports/invoice') return 'Invoice';
      ...
      <aside className={`rs-sidebar ${sidebarOpen ? '' : 'collapsed'} ${mobileSidebarOpen ? 'mobile-open' : ''}`}>
        ...
        <div className="rs-sidebar-logo">
          <div className="rs-logo-text">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="28" height="28" style={{ color: '#10b981' }}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            {showLabels && <span className="rs-logo-label">Adsense Admin</span>}
          </div>
        </div>

        {/* Navigation */}
        <nav className="rs-sidebar-nav">
          {showLabels && <div className="rs-nav-section-label">Dashboard</div>}
          {dashboardItems.map(renderNavItem)}

          {showLabels && <div className="rs-nav-section-label">Reports</div>}
          {reportItems.map(renderNavItem)}

          {showLabels && <div className="rs-nav-section-label">Other</div>}
          ...
          <div className="rs-topbar-right">
            <div className="rs-profile-area" ref={profileRef}>
              ...
              >
                ...
                <span className="rs-profile-name">{username || 'Admin'}</span>
              </button>
              ...
              {profileOpen && (
                <div className="rs-profile-dropdown">
                  <div className="rs-profile-dropdown-header">
                    ...
                    <div>
                      <div className="rs-profile-dropdown-name">{username || 'Admin'}</div>
                      <div className="rs-profile-dropdown-role">Administrator</div>
                    </div>
                  </div>
